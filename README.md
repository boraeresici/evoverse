...
- **Chirality field & earned eras** — a symmetry-breaking *maturity* subsystem inspired by Ozturk & Sasselov's homochirality research: a universe-wide field (their magnetized surface) biases every region's pitchfork the same way, so the hand that breaks is contingent but **global**; it avalanches and locks, lineages inherit it one-way (with a lethal flip mutation), and the universe's `homochirality_index` — *global* single-handedness, `|mean ee|`, not `mean |ee|` — gates a monotonic `Genesis → Expansion → Stabilization → Intelligence` progression that must be **earned**, not seeded. Turn the field off and a universe locks every region yet never agrees on a hand, and is correctly denied life. See [`docs/CHIRALITY_AND_MIND.md`](docs/CHIRALITY_AND_MIND.md).
